...
def find_path_to(maps, pos: tuple, locations, traversable: List):
    # a path which is too long is not practical when moving one by one
    shortest_path_length = MAX_STEPS + 1
    shortest_path = None
...
def find_nearest_spoil(games: GameState, player: Player):
    ...
    return find_path_to(maps, (px, py), spoil_locations, [TerrainType.ROAD.value])
...
def plant_bomb_near_balk(games: GameState, player: Player):
    ...
    return find_path_to(maps, (px, py), bombable_locations, [TerrainType.ROAD.value])
...
def move_away_from_bomb(games: GameState, player: Player):
    ...
    return find_path_to(maps, (px, py), road_locations, [TerrainType.ROAD.value, TerrainType.ROAD_WITH_BOMB.value])
